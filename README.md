...
3. **【Ultra 新增】课表排版极致优化**：抛弃传统 Flex 布局，底层采用 **CSS Grid 网格布局**，严格保证代表每一周的 16 个小格宽度绝对相等，彻底解决文字长度和边框导致的图块不对齐问题；同时在半学期（第8/9周）正中间加入了独立的参考细线，兼顾美观与实用；列与列之间加入粗线，课表文字增加白边高亮，彻底治愈重度强迫症。
4. **【Ultra 新增】智能交互**：在选课列表点击课程标题即可一键搜索该课；在右侧课表中点击具体的课程色块，选课列表会自动平滑滚动定位到该课程并高亮显示。这样极度方便了冲突换课。
